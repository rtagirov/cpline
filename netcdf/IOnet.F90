 ...
 status = nf_def_var( ncid, trim(varname),  NF_REAL, numberdims ,dims, varid )
 if ( status .ne. NF_NOERR ) call  handle_error(status)
 ...
 subroutine write_netcdf(ncid, myrank, nproc, varname,  var , nlayer,  n1, n2, n3, comm,  ier)
  ...
  integer ncount, vartag
   ...
   if (myrank .eq. 0) then
     ...
     call MPI_Send(var, ncount, MPI_REAL, 0, vartag, comm, ier)
   ...
   do i = 1, n3
      do j = 1, n2
         ...
         icount3(3) = 1
          ...
          status = nf_put_vara_real(ncid,varid,istart3, icount3, var(1,j,i) )
          if ( status .ne. NF_NOERR) call handle_error(status)
...
! nf_inq_dimid returns dimension ID (integer) of "string" in ncid
   ...
   if ( myrank .eq. 0 ) then
    ...
    call  MPI_Irecv(var(1,1,1), ncount, MPI_real, 0, vartag, comm, ireq, ier)
   ...
   do i = 1, n3
      do j = 1, n2
         ...
         icount3(3) = 1
          ...
          status = nf_get_vara_real(ncid,varid,istart3, icount3, var(1,j,i) )
          if ( status .ne. NF_NOERR) call handle_error(status)
